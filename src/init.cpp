...
void Shutdown()
{
    ...
    RenameThread("DingoDollars-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: DingoDollars.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: DingoDollarsd.pid)") + "\n";
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 31173)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeDingoDollarsamount=<n> " + _("Keep N DingoDollars anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. DingoDollars is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. DingoDollars is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("DingoDollars version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "DingoDollars server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of DingoDollars") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart DingoDollars to complete") << "\n";
    ...
    nAnonymizeDingoDollarsAmount = GetArg("-anonymizeDingoDollarsamount", 0);
    if(nAnonymizeDingoDollarsAmount > 999999) nAnonymizeDingoDollarsAmount = 999999;
    if(nAnonymizeDingoDollarsAmount < 2) nAnonymizeDingoDollarsAmount = 2;
    ...
    LogPrintf("Anonymize DingoDollars Amount %d\n", nAnonymizeDingoDollarsAmount);
