...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeDingoDollarsAmount"))
        settings.setValue("nAnonymizeDingoDollarsAmount", 1000);
    nAnonymizeDingoDollarsAmount = settings.value("nAnonymizeDingoDollarsAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeDingoDollarsAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeDingoDollarsAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeDingoDollarsAmount:
            return QVariant(nAnonymizeDingoDollarsAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeDingoDollarsAmount:
            nAnonymizeDingoDollarsAmount = value.toInt();
            settings.setValue("nAnonymizeDingoDollarsAmount", nAnonymizeDingoDollarsAmount);
            emit AnonymizeDingoDollarsAmountChanged(nAnonymizeDingoDollarsAmount);
            break;
        default:
            break;
        }
