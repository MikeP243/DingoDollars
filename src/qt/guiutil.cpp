...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("DingoDollars"))
        return false;
...
{
    // Convert DingoDollars:// to DingoDollars:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("DingoDollars://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "DingoDollars:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "DingoDollars.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for DingoDollars.lnk
...
{
    return GetAutostartDir() / "DingoDollars.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a DingoDollars.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=DingoDollars\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the DingoDollars app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add DingoDollars app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("DingoDollars-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  DingoDollars-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("DingoDollars-Qt"));
