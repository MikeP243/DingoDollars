...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeDingoDollarsAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDingoDollarsAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeDingoDollarsAmount = strAnonymizeDingoDollarsAmount.remove(strAnonymizeDingoDollarsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDingoDollarsAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeDingoDollarsAmount*COIN) nMaxToAnonymize = nAnonymizeDingoDollarsAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeDingoDollarsAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeDingoDollarsAmount));
        strAnonymizeDingoDollarsAmount = strAnonymizeDingoDollarsAmount.remove(strAnonymizeDingoDollarsAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDingoDollarsAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeDingoDollarsAmount == 0){
