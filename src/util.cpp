...
int nAnonymizeDingoDollarsAmount = 1000;
...
#else
    const char* pszModule = "DingoDollars";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\DingoDollars
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\DingoDollars
    // Mac: ~/Library/Application Support/DingoDollars
    // Unix: ~/.DingoDollars
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "DINGO";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "DingoDollars.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "DingoDollarsd.pid"));
